...
void Lisp_init(){
  si_safe_eval(3, c_string_to_object("(cl-user::init)"), Cnil, OBJNULL);
}

void Lisp_main_loop_iteration(){
  //cl_funcall(1, c_string_to_object("main-loop-iteration"));
  si_safe_eval(3, c_string_to_object("(cl-user::main-loop-iteration)"), Cnil, OBJNULL);
}

void Lisp_set_window_size(int width, int height){
  char tmp[512];
  sprintf(tmp, "(cl-user::set-window-size %d %d)", width, height);
  si_safe_eval(3, c_string_to_object(tmp), Cnil, OBJNULL);
}

void Lisp_register_touch(float x, float y){
  char tmp[512];
  sprintf(tmp, "(cl-user::register-touch %f %f)", x, y);
  si_safe_eval(3, c_string_to_object(tmp), Cnil, OBJNULL);
}
